...
with st.sidebar:
    ...
    temperature = st.sidebar.slider('temperature', min_value=0.01, max_value=5.0, value=0.1, step=0.1, help="The higher the temperature, the more random the output.")
...
llm = initialize_llm(model, temperature, context_window)

@st.cache_resource(show_spinner=False)
def load_data(model, temperature, context_window):
    """
    ...
    with st.spinner(text="Loading and indexing the news articles that you just scrapped – hang tight! This should take 10 seconds to 1 minute."):
        if st.session_state.get("all_results"):
            ...
            for i in range(len(st.session_state.all_results)):
                if st.session_state.all_results[i].get('scrapped_text') != None:
                    text_list.append(st.session_state.all_results[i].get('scrapped_text'))
            st.session_state["loaded_tokens"] = sum(len(text.split()) for text in text_list)
            documents = [Document(text=t, meta={'date': st.session_state.all_results[i].get('date'), 'url': st.session_state.all_results[i].get('URL')}) for i, t in enumerate(text_list)]
...
if activate:
    query_engine = load_data(model, temperature, context_window)
    st.sidebar.markdown(f"*Loaded {st.session_state['loaded_tokens']} words from {len(st.session_state.all_results)} articles*")
